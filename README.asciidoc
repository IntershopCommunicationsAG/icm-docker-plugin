...
applied to the project. Version {latestRevision} of this plugin is the minimum version.
...
| `intershop.db.mssql.autoRemoveContainer` | controls if the container is created with `autoRemove==true` (remove container when stopped) | Boolean | Optional | `true` +
...
| `webserver.http.port` | the host port to be used for the WebAdapter http port. To configure on a non-default port (e.g. `80`), set `webserver.container.http.port` to the same value. | Integer | Optional | `8080` +
| `webserver.https.port` | the host port to be used for the WebAdapter https port. To configure on a non-default port (e.g. `443`) with correct secure URL generation, set `webserver.container.https.port` to the same value. | Integer | Optional | `8443` +
| `webserver.container.http.port` | the container port the WebAdapter listens on and advertises for http URLs (via `ICM_WA_HTTP_PORT`). Set it equal to `webserver.http.port` when configuring a non-default http port. When a privileged container port (< 1024, e.g. `80`) is configured, the plugin automatically grants the WA container the `NET_BIND_SERVICE` capability so the non-root process can bind it. | Integer | Optional | `8080` +
| `webserver.container.https.port` | the container port the WebAdapter listens on and advertises for secure URLs (via `ICM_WA_HTTPS_PORT`). Set it equal to `webserver.https.port` when configuring a non-default https port so generated secure links use that port instead of the default `8443`. When a privileged container port (< 1024, e.g. `443`) is configured, the plugin automatically grants the WA container the `NET_BIND_SERVICE` capability so the non-root process can bind it. | Integer | Optional | `8443` +
...
| `wa.autoRemoveContainer` | controls if the container is created with `autoRemove==true` (remove container when stopped) | Boolean | Optional | `true` +
|===

NOTE: `webserver.http.port` / `webserver.https.port` are the *host* ports Docker configures, while `webserver.container.http.port` / `webserver.container.https.port` are the ports the WebAdapter listens on and uses to build absolute URLs (links, redirects, server name). With the defaults (`8080`/`8443`) they match. To configure a non-default port (e.g. `80`/`443`), set the matching container port to the same value; otherwise the WebAdapter advertises its internal port (e.g. `:8443`) in generated links, which the browser cannot reach.
...
| `waa.autoRemoveContainer` | controls if the container is created with `autoRemove==true` (remove container when stopped) | Boolean | Optional | `true` +
|===
...
| `nginx.autoRemoveContainer` | controls if the container is created with `autoRemove==true` (remove container when stopped) | Boolean | Optional | `true` +
|===
...
|===
| Key | Description | Co-domain | Mandatory/Optional | Default value
| `solr.data.folder.path` | host folder to store Solr data outside of the container | String | Optional | <empty> +
| `solr.nodes` | number of Solr nodes to start. When greater than `1`, a `solr-lb` nginx load-balancer container is started. | Integer | Optional | `1` +
| `solr.cloudSolrServerURLs` | preferred Solr HTTP endpoint configuration. Use one or more comma-separated URLs, for example `http://solr.example.com:8983/solr` or `http://solr1:8983/solr,http://solr2:8984/solr`. | URL | Optional | automatic default +
| `solr.zooKeeperHostList` | legacy Solr connection configuration. Used when `solr.cloudSolrServerURLs` is not configured. | String | Optional | <empty> +
...
| `solr.lb.port` | the host port to be used for the `solr-lb` HTTP port when `solr.nodes` is greater than `1` | Integer | Optional | `8990` +
| `solr.useHostUser` | controls if the host system's current user is used for the container start | Boolean | Optional | `false` +
| `solr.lb.useHostUser` | controls if the host system's current user is used for the `solr-lb` container start | Boolean | Optional | `false` +
| `solr.autoRemoveContainer` | controls if the container is created with `autoRemove==true` (remove container when stopped) | Boolean | Optional | `true` +
| `solr.lb.autoRemoveContainer` | controls if the `solr-lb` container is created with `autoRemove==true` (remove container when stopped) | Boolean | Optional | `true` +
|===

When both connection properties are configured, `solr.cloudSolrServerURLs` takes precedence. If neither is configured, the generated default is `http://<solr-host>:8983/solr` for one node, or `http://<solr-lb-host>:<solr.lb.port>/solr` when `solr.nodes` is greater than `1`.
...
|===
| Key | Description | Co-domain | Mandatory/Optional | Default value
| `solr.data.folder.path` | host folder to store Zookeeper data outside of the container in `<solr.data.folder.path>/zk` | String | Optional | <empty> +
...
| `zookeeper.useHostUser` | controls if the host system's current user is used for the container start | Boolean | Optional | `false` +
| `zookeeper.autoRemoveContainer` | controls if the container is created with `autoRemove==true` (remove container when stopped) | Boolean | Optional | `true` +
|===
...
| `mail.autoRemoveContainer` | controls if the container is created with `autoRemove==true` (remove container when stopped) | Boolean | Optional | `true` +
|===
...
| `redis.autoRemoveContainer` | controls if the container is created with `autoRemove==true` (remove container when stopped) | Boolean | Optional | `true` +
|===
...
| `intershop.file.monitoring.polling` | If not specified or set to `false` the ICM configuration property `intershop.file.monitoring.polling` is set to `true` allowing the icm-as to work make all the `*.CheckSource` properties to work. To enforce the icm-as to work with `intershop.file.monitoring.polling=false` explicitly set `intershop.file.monitoring.polling=false` in `icm.properties`. | Boolean | Optional | `true` +
| `intershop.file.monitoring.pollInterval` | The value of this property is transferred to the icm-as independent of the value of the property `intershop.file.monitoring.polling` allowing to set the polling interval (if enabled). It specifies the polling interval in milliseconds. | Integer | Optional | `1000` +
|===
...
| `classpathLayout` | sets a custom classpath layout + | comma separated list of {`release`,`source`,`sourceJar`,`eclipse`}, default value is `sourceJar,release` | `sourceJar,release` +
| `mount` | mount a host directory onto a container volume + | String[], each value following the pattern `hostPath:containerPath`, can be used multiple times to mount multiple directories | _none_ +
|===
NOTE: To make the `classpathLayout` `sourceJar` work the gradle-property `localVersion` has to be set to `true` (see https://github.com/IntershopCommunicationsAG/gitflow-version-gradle-plugin#gradle-properties).

NOTE: Using `--mount` one can achieve to let the startup scripts import custom SSL/TLS certificates into the JVM's _cacerts_. Ensure to use certificate files using the `PEM` format and having the file extension `crt`. The required environment variable `USE_SYSTEM_CA_CERTS=1` is set automatically (further details at https://hub.docker.com/r/amd64/eclipse-temurin/#can-i-add-my-internal-ca-certificates-to-the-truststore).

NOTE: When using Windows as the host system ensure to use `\` (backslashes) in your host path.
++++
<details>
<summary>Examples</summary>
++++
...
----
.start application server and mount custom SSL/TLS certificates to `/certificates`
[source,shell]
----
./gradlew createAS --mount c:\my-icm\custom-certificates:/certificates startAS
----
...
IMPORTANT: As long as the application server is starting the container's log output is forwarded to the host's standard out. Afterward, it is only accessible by `docker logs`.
...
| Task | Property Key | Property Value
| *ZK + 
*Solr | solr.data.folder.path | host folder to store Solr and Zookeeper data outside of the Solr/Zookeeper container. e.g. `D:/solr-data` 

.2+| cleanUpSolr +
listSolr| Solr connection properties | See <<SolrConfiguration>> for the preferred URL property, legacy ZooKeeper property, precedence, and automatic defaults.
...
For the configuration on a special local machine it is necessary to provide an `icm.properties`-file in file structure.
It is possible to configure the location with environment properties. If a certain property is requested but *not found* in the
`icm.properties`-file the gradle system is asked for the property. That allows to specify such a property

- on the command line (e.g. `./gradlew startAS -Pwa.autoRemoveContainer=false`)
- in one of the `gradle.properties`-files

This is a minimum set of properties in `icm.properties`:
...
# solr configuration
# Preferred: Solr HTTP endpoint(s) used by ICM and Solr administration tasks.
# For multiple endpoints, separate URLs with commas.
solr.cloudSolrServerURLs = http://localhost:8983/solr

# Legacy alternative. If both properties are set, solr.cloudSolrServerURLs wins.
...
=== Setting icm-as environment variables
For the task `createAS`, which creates a Docker container for all the appserver starts (tasks: `startAS`, `dbPrepare`, `startASTestContainer`), there is a special handling of properties configured in the `icm.properties` file. Properties with keys starting with `intershop.environment.` are provided as environment variables to the container:

* If the property's value contains no `=` at all, the key is transformed as follows: `intershop.environment.<symbolicName>` -> `<symbolicName>`
* If the property's value contains at least 1 `=`, the key is transformed as follows: `intershop.environment.<symbolicName>` -> `ISH_ENV_<symbolicName>`

NOTE: Environment variables with keys following the pattern `ISH_ENV_<symbolicName>` are picked up by the icm-as configuration finder `system`. Their value is split at `,` and all the parts are provided as separate configuration values.

NOTE: Other environment variables may also be picked up by the icm-as configuration finder `system` if their key matches a requested configuration property's name when transformed to uppercase and all `.` are replaced by `_` (example: `intershop.encryption.strictMode.enabled` -> `INTERSHOP_ENCRYPTION_STRICTMODE_ENABLED`).
