...
| `webserver.container.http.port` | the container port the WebAdapter listens on and advertises for http URLs (via `ICM_WA_HTTP_PORT`). Set it equal to `webserver.http.port` when configuring a non-default http port. | Integer | Optional | `8080` +
