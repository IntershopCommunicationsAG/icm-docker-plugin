...
|`icmsetup`   | `Property<String>` | `docker.intershop.de/intershop/icm-as:latest`       | This image is used as a base image for the most ICM images.
...
| *start** | `StartExtraContainerTask` | Start image with the necessary configuration.
| *stop** | `StopExtraContainerTask` | Stop image by name.
...
.2+| WA | `webserver.http.port` | Exposed http port of the webserver.
...
|===
|Property               | Type      | Default value | Description
|`ishUnitTests` | `NamedDomainObjectContainer<Suite>`  | | This is a configuration for special Intershop server unit tests. This configuration is used only in a project.
|===
