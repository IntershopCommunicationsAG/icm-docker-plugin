...
.3+| cleanUpSolr +
listSolr| `solr.cloudSolrServerURLs` | Preferred Solr HTTP endpoint configuration. Use one or more comma-separated Solr URLs, e.g. `http://solr.example.com:8983/solr` or `http://solr1:8983/solr,http://solr2:8984/solr`.
| `solr.zooKeeperHostList` | Legacy Solr connection configuration. A list of Zookeeper hosts with port and an optional Zookeeper root. +
e.g. zookeeper1:2181;zookeeper2:2181;zookeeper3:2181/solr
If neither connection property is configured, ICM uses `http://<solr-container>:8983/solr` for one node and `http://<solr-lb-container>:80/solr` when `solr.nodes` is greater than one.
