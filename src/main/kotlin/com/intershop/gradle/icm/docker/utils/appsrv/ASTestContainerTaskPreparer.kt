...
class ASTestContainerTaskPreparer(
        project: Project,
        val networkTask: Provider<PrepareNetwork>,
) : AbstractASTaskPreparer(project, networkTask) {

    companion object {
        const val EXT_NAME: String = "AsTestContainer"
    }

    override fun getExtensionName(): String = EXT_NAME

    /**
     * Calculates the image from the output of the task [ICMDockerPlugin.BUILD_TEST_IMAGE]
     */
    ...
    init {
        ...
        registerStartContainerTask(createTask, StartServerContainer::class.java).configure { task ->
            task.description = """
                Starts Production Application Server in a container - only for use in icm-as.
                ATTENTION: task ${
                taskNameOf("create")
