...
@Inject constructor(
...
) : DockerCreateContainer(objectFactory) {
    ...
    val createdContainer: Property<ContainerHandle> =
            objectFactory.property(ContainerHandle::class.java).convention(providerFactory.provider {
     ...
     */
    @Internal
    fun getPrimaryPortMapping(): Provider<PortMapping> = providerFactory.provider {
    ...
    fun withPortMappings(vararg portMappings: PortMapping) {
        portMappings.forEach { currPortMapping ->
            ...
            hostConfig.portBindings.add(providerFactory.provider { currPortMapping.render() })
        }
    }
    ...
    fun withEnvironment(environment: ContainerEnvironment) {
        withEnvironment(providerFactory.provider { environment })
    }
    ...
    fun withEnvironment(environment: Provider<ContainerEnvironment>) {
        envVars.putAll(
                providerFactory.provider { // use a provider around environment.get() to defer the execution until actual usage
                    environment.get().toMap()
                })
    }

    fun withVolumes(volumes: Map<String, String>) {
        withVolumes(providerFactory.provider { volumes })
    }
    ...
    init {
        this.onlyIf("Container does not exist") {
            ...
            if (currentContainerState.exists()) {
                logger.quiet("{} still exists, skipping creation", existingContainer.get())
                return@onlyIf false
            }
            return@onlyIf true
        }
